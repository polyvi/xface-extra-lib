...
pbxProject.prototype.removeFromPbxFileReferenceSection = function (file) {
    ...
    for(i in this.pbxFileReferenceSection()) {
        var section = this.pbxFileReferenceSection()[i];
        if(section.name && section.name.indexOf('"') == -1) {
            section.name = '"' + section.name + '"';
        }
        if(section.path && section.path.indexOf('"') == -1) {
            section.path = '"' + section.path + '"';
        }
        if(section.name == refObj.name ||
           section.path == refObj.path) {
